---
title: "explore_clustering_parameters_train_highSMA"
...
dat_imputed_SMA        <- filter(dat_imputed, mcsid_cm %in% mcsid_cm_SMA$mcsid_cm) %>%
  left_join(mcsid_cm_SMA, by = "mcsid_cm") 
# Then separate by the answer
dat_imputed_highSMA     <- filter(dat_imputed_SMA, sma == "Agree" | sma == "Strongly agree") %>%
  dplyr::select(-sma)
...
# Data setup
mds_data     <- dplyr::select(dat_imputed_highSMA, -mcsid_cm)
...
FigS01d_plot <- ggplot(gmm_all, aes(x = G, y = BIC, group = Dimensions, color = as.factor(Dimensions))) +
  geom_line() + geom_point() +
...
ggsave("figures/FigS01d.png", plot = FigS01d_plot, bg = "white", width = 7, height = 4.5, units = "in")
...
FigS01e_plot <- ggplot(gmm_all, aes(x = G, y = Silhouette, group = Dimensions, color = as.factor(Dimensions))) +
  geom_line() + geom_point() +
...
ggsave("figures/FigS01e.png", plot = FigS01e_plot, bg = "white", width = 7, height = 4.5, units = "in")
...
FigS01f_plot <- ggplot(gmm_all, aes(x = G, y = FuzzySilhouette, group = Dimensions, color = as.factor(Dimensions))) +
  geom_line() +
  geom_point() +
  labs(
    title = "Fuzzy Silhouette Score vs. Number of Clusters (G)",
    x = "Number of Clusters (G)",
    y = "Mean Fuzzy Silhouette Score",
    color = "MDS Dimensions (k)"
  ) +
  scale_color_viridis_d(option = "plasma") +
  theme_minimal()
...
ggsave("figures/FigS01f.png", plot = FigS01f_plot, bg = "white", width = 7, height = 4.5, units = "in")
