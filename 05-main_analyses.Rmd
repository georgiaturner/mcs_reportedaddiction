...
set.seed(20230427) # set seed for any random samplings
...
First we define some plotting parameters, including variable renaming and colour scheme to add to plots throughout.
...
In the previous scripts ('explore_clustering_parameters_train...') we examined different dimensionality reduction and clustering parameters, and established that clusters emerge in the training dataset, for both all and high SMA, at dimensionality reduction to 2 MDS dimensions, with 2 clusters providing separation using Gaussian mixture modelling for all participants, and 3 clusters for high SMA.

Therefore, in this script we perform clustering using these analysis variables (MDS dimensions = 2, clusters = 2 for all; 3 for high SMA). 

First we do MDS. We perform MDS to extract dimensions from the training dataset, then project the testing dataset onto these dimensions.
...
# S02a shows only the first 30 most important dimensions for ease of visualisation; S03a shows all dimensions.
...
ggsave("figures/FigS02a.pdf", plot = FigS02a_plot, bg = "white",  width = 7, height = 4.5, units = "in")
...
ggsave("figures/FigS03a.pdf", plot = FigS03a_plot, bg = "white",  width = 7, height = 20, units = "in")
...
ggsave("figures/Fig02a.pdf", plot = Fig02a_plot, bg = "white", width = 7, height = 4.5, units = "in")
...
ggsave("figures/FigS03b.pdf", plot = FigS03b_plot, bg = "white",  width = 7, height = 20, units = "in")
...
In each case, we extract the cluster boundaries from the training dataset, and use the boundaries in the testing dataset.
...
dat_imputed_all_train_gmm2     <- Mclust(dat_imputed_all_train_mds$points, G = 2)
dat_imputed_all_test_gmm2      <- predict(dat_imputed_all_train_gmm2, newdata = dat_imputed_all_test_mds_points)

dat_imputed_highSMA_train_gmm3 <- Mclust(dat_imputed_highSMA_train_mds$points,  G = 3)
dat_imputed_highSMA_test_gmm3  <- predict(dat_imputed_highSMA_train_gmm3, newdata = dat_imputed_highSMA_test_mds_points)

# Relabel all so that Cluster 1 has highest wellbeing

dat_imputed_all_train_gmm2 <- relabel_gmm_by_wellbeing(
  dat_imputed_all_train_mds$points,
  dat_imputed_all_train_gmm2,
  warwick_wellbeing = dat_imputed_all_train$warwick_wellbeing, 
  G = 2
)

dat_imputed_all_test_gmm2 <- relabel_gmm_by_wellbeing(
  dat_imputed_all_test_mds_points,
  dat_imputed_all_test_gmm2,
  warwick_wellbeing = dat_imputed_all_test$warwick_wellbeing, 
  G = 2
)

dat_imputed_highSMA_train_gmm3 <- relabel_gmm_by_wellbeing(
  dat_imputed_highSMA_train_mds$points,
  dat_imputed_highSMA_train_gmm3,
  warwick_wellbeing = dat_imputed_highSMA_train$warwick_wellbeing, 
  G = 3
)

dat_imputed_highSMA_test_gmm3 <- relabel_gmm_by_wellbeing(
  dat_imputed_highSMA_test_mds_points,
  dat_imputed_highSMA_test_gmm3,
  warwick_wellbeing = dat_imputed_highSMA_test$warwick_wellbeing, 
  G = 3
)
...
plot_clusters_train_all <- plot_mds_clusters(mds_result = dat_imputed_all_train_mds$points, 
                                             gmm_model_points  = dat_imputed_all_train_gmm2,
                                             gmm_model_ellipses  = dat_imputed_all_train_gmm2,

                                             title  = "Clustering for train all",
                                             cluster_colours = all_cluster_colours
...
plot_clusters_test_all <- plot_mds_clusters(mds_result = dat_imputed_all_test_mds_points, 
                                             gmm_model_points = dat_imputed_all_test_gmm2,
                                            gmm_model_ellipses = dat_imputed_all_train_gmm2,
...
FigS02b_plot <- plot_clusters_test_all

ggsave("figures/FigS02b.pdf", plot = FigS02b_plot, bg = "white", width = 14, height = 9, units = "in")
...
plot_clusters_highSMA_train_gmm3 <- plot_mds_clusters(mds_result = dat_imputed_highSMA_train_mds$points, 
                                             gmm_model_points = dat_imputed_highSMA_train_gmm3,
                                             gmm_model_ellipses = dat_imputed_highSMA_train_gmm3,
...
plot_clusters_highSMA_test_gmm3 <- plot_mds_clusters(mds_result = dat_imputed_highSMA_test_mds_points, 
                                             gmm_model_points = dat_imputed_highSMA_test_gmm3,
                                             gmm_model_ellipses = dat_imputed_highSMA_train_gmm3,
...
ggsave("figures/Fig02b.pdf", plot = Fig02b_plot, bg = "white", width = 14, height = 9, units = "in")
...
ggsave("figures/FigS02c.pdf", plot = FigS02c_plot, bg = "white", width = 6, height = 8, units = "in")
...
ggsave("figures/Fig01b.pdf", plot = Fig01b_plot, bg = "white", width = 7, height = 4.5, units = "in")

``` 
As an exploratory analysis, we want to understand the behavioural-emotional make-up of each cluster. To do so, we use elastic net regularised regression to predict membership of each cluster (1) compared to those within other SMA clusters, and (2) compared to those without SMA. 

(1) We begin by using regression to predict membership of each cluster compared to other clusters within the SMA group:
...
ggsave("figures/Fig03a.pdf", plot = Fig03a_plot, bg = "white", width = 7, height = 4.5, units = "in")
ggsave("figures/Fig03b.pdf", plot = Fig03b_plot, bg = "white", width = 7, height = 4.5, units = "in")
ggsave("figures/Fig03c.pdf", plot = Fig03c_plot, bg = "white", width = 7, height = 4.5, units = "in")


``` 

As a sensitivity analysis to see if the features are simply predicting SMA severity within the high SMA group rather than cluster membership, we see what happens within the SMA group if we just predict SMA answer (i.e., either Strongly agree or Agree). We find no predictors indicating the cluster differences are not just explained by SMA severity.
...
(2) Next, we use regression to predict membership of each cluster compared to those NOT in the SMA group:
...
fit_gmm3_C1 <- cv.glmnet(as.matrix(dat_gmm3_C1_xtrain), 
                         as.matrix(dat_gmm3_C1_ytrain),
                         type.measure="mse", 
                         alpha=0.5, 
                         family="binomial")
...
ggsave("figures/Fig03d.pdf", plot = Fig03d_plot, bg = "white", width = 7, height = 4.5, units = "in")
ggsave("figures/Fig03e.pdf", plot = Fig03e_plot, bg = "white", width = 7, height = 4.5, units = "in")
ggsave("figures/Fig03f.pdf", plot = Fig03f_plot, bg = "white", width = 7, height = 4.5, units = "in")
...
Now create dummy variables for the unordered factors.
...
Now perform imputation for the remaining variables.
...
#### MAKE TABLES
 ...
 # Row function:
make_prop_N_row <- function(df, cluster_var, cat_var) {
  ...
  props  <- round(as.data.frame.matrix(prop_tab), 3)
  
  row_final <- data.frame(Category = rownames(counts))

  for (k in seq_len(ncol(counts))) {
    row_final[[paste0("C", k, "_N")]]    <- counts[, k]
    row_final[[paste0("C", k, "_prop")]] <- props[, k]
    }
...
make_SES_row <- function(df, cluster_var, ses_var) {
  ...
  stats <- df %>%
    ...
    dplyr::summarise(
      n_val    = dplyr::n(),
      mean_val = mean(!!rlang::sym(ses_var), na.rm = TRUE) %>% round(3),
      .groups = "drop"
    ) %>%
    dplyr::arrange(!!rlang::sym(cluster_var))
  
  row_final <- data.frame(Category = "SES PC1")

  # Add group means dynamically
  for (i in seq_len(nrow(stats))) {
    row_final[[paste0("n_C", i)]] <- stats$n_val[i]
    row_final[[paste0("C", i)]]   <- stats$mean_val[i]
  }
...
summary_all_cluster_train <- bind_rows(
  make_prop_N_row(dat_train_demographics_SES, "all_cluster", "gender_id_recoded"),
  make_prop_N_row(dat_train_demographics_SES, "all_cluster", "five_level_ethnicity"),
  make_SES_row(dat_train_demographics_SES, "all_cluster", "SES_PC1")
)
...
summary_highSMA_cluster_train <- bind_rows(
  make_prop_N_row(dat_train_demographics_SES, "highSMA_cluster", "gender_id_recoded"),
  make_prop_N_row(dat_train_demographics_SES, "highSMA_cluster", "five_level_ethnicity"),
  make_SES_row(dat_train_demographics_SES, "highSMA_cluster", "SES_PC1")
)

summary_all_cluster_test <- bind_rows(
  make_prop_N_row(dat_test_demographics_SES, "all_cluster", "gender_id_recoded"),
  make_prop_N_row(dat_test_demographics_SES, "all_cluster", "five_level_ethnicity"),
  make_SES_row(dat_test_demographics_SES, "all_cluster", "SES_PC1")
)
...
summary_highSMA_cluster_test <- bind_rows(
  make_prop_N_row(dat_test_demographics_SES_highSMA, "highSMA_cluster", "gender_id_recoded"),
  make_prop_N_row(dat_test_demographics_SES_highSMA, "highSMA_cluster", "five_level_ethnicity"),
  make_SES_row(dat_test_demographics_SES_highSMA, "highSMA_cluster", "SES_PC1")
)
...
# High SMA, Gender v. cluster:
print(
  compute_chisq(dat_test_demographics_SES_highSMA, "highSMA_cluster", "gender_id_recoded"))
# Post hoc tests:
C1_N_female <- filter(summary_highSMA_cluster_test, Category == "Female")$C1_N
C2_N_female <- filter(summary_highSMA_cluster_test, Category == "Female")$C2_N
C3_N_female <- filter(summary_highSMA_cluster_test, Category == "Female")$C3_N

C1_N_total_gender  <- sum(make_prop_N_row(dat_test_demographics_SES_highSMA, "highSMA_cluster", "gender_id_recoded")$C1_N)
C2_N_total_gender  <- sum(make_prop_N_row(dat_test_demographics_SES_highSMA, "highSMA_cluster", "gender_id_recoded")$C2_N)
C3_N_total_gender  <- sum(make_prop_N_row(dat_test_demographics_SES_highSMA, "highSMA_cluster", "gender_id_recoded")$C3_N)

GenderC1C2 <- matrix(
  c(C1_N_female, C1_N_total_gender - C1_N_female,
    C2_N_female, C2_N_total_gender - C2_N_female),
  nrow = 2,
  byrow = TRUE
)
chisq.test(GenderC1C2)

GenderC1C3 <- matrix(
  c(C1_N_female, C1_N_total_gender - C1_N_female,
    C3_N_female, C3_N_total_gender - C3_N_female),
  nrow = 2,
  byrow = TRUE
)
chisq.test(GenderC1C3)

GenderC2C3 <- matrix(
  c(C2_N_female, C2_N_total_gender - C2_N_female,
    C3_N_female, C3_N_total_gender - C3_N_female),
  nrow = 2,
  byrow = TRUE
)
chisq.test(GenderC2C3)
...
# High SMA, Ethnicity v. cluster:
print(
  compute_chisq(dat_test_demographics_SES_highSMA, "highSMA_cluster", "five_level_ethnicity"))
# Not significant so no post-hoc tests

# High SMA, SES PC1 v. cluster:
SES_anova <- aov(dat_test_demographics_SES_highSMA[["SES_PC1"]] ~ as.factor(dat_test_demographics_SES_highSMA[["highSMA_cluster"]]), data = dat_test_demographics_SES_highSMA)
summary(SES_anova)
# Post hoc tests:
TukeyHSD(SES_anova)
...
Now we compare each variable between the clusters (Figure 4).

Figure 4a: Comparing self-reported addiction between clusters.
...
ggsave("figures/Fig04a.pdf", plot = Fig04a_plot, bg = "white", width = 7, height = 4.5, units = "in")
...
c1_prop_Agree <- mean(
  filter(dat_test_smHrs, cluster == "highSMA1")$sma == "Agree"
)
c2_prop_Agree <- mean(
  filter(dat_test_smHrs, cluster == "highSMA2")$sma == "Agree"
)
c3_prop_Agree <- mean(
  filter(dat_test_smHrs, cluster == "highSMA3")$sma == "Agree"
)
...
Figure 4b: comparing the relationship between time spent on social media and SMA.
...
ggsave("figures/Fig04b.pdf", plot = Fig04b_plot, bg = "white", width = 7, height = 4.5, units = "in")
...
compare_kendall_clusters <- function(kendall_results, cluster1, cluster2) {

  z1 <- filter(kendall_results, cluster == cluster1)$z
  z2 <- filter(kendall_results, cluster == cluster2)$z

  n1 <- filter(kendall_results, cluster == cluster1)$n
  n2 <- filter(kendall_results, cluster == cluster2)$n
  ...
  # Print results
  cat("\n")
  cat(cluster1, "vs", cluster2, "\n")
  cat("Z-score:", z_score, "\n")
  cat("P-value:", p_value, "\n")
  cat("P-Bonf:", p_Bonf, "\n")


  # Return values invisibly if needed later
  invisible(list(
...
compare_kendall_clusters(kendall_results, "highSMA1", "highSMA2")
compare_kendall_clusters(kendall_results, "highSMA1", "highSMA3")
compare_kendall_clusters(kendall_results, "highSMA2", "highSMA3")
compare_kendall_clusters(kendall_results, "highSMA1", "lowSMA")
compare_kendall_clusters(kendall_results, "highSMA2", "lowSMA")
compare_kendall_clusters(kendall_results, "highSMA3", "lowSMA") 



``` 

Figure 4c and d: Comparing depression and self control.
...
ggsave("figures/Fig04c.pdf", plot = Fig04c_plot, bg = "white", width = 7, height = 4.5, units = "in")
ggsave("figures/Fig04d.pdf", plot = Fig04d_plot, bg = "white", width = 7, height = 4.5, units = "in")

####################################################################
#### Now do stats.
####################################################################

# Pairwise comparisons for kessl_depression.y
Dep_anova <- aov(kessl_depression.y ~ cluster, data = dat_test_smHrs)
summary(Dep_anova)
# Post hoc tests:
TukeyHSD(Dep_anova)
# Get means for each profile:
emmeans(Dep_anova, pairwise ~ cluster, adjust = "bonferroni")


# Pairwise comparisons for self_control.y
SC_anova <- aov(self_control.y ~ cluster, data = dat_test_smHrs)
summary(SC_anova)
# Post hoc tests:
TukeyHSD(SC_anova)
# Get means for each profile:
emmeans(SC_anova, pairwise ~ cluster, adjust = "bonferroni")
